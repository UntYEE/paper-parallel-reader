...
1. 在页面左上方输入论文标题、关键词、arXiv ID 或 PDF 链接，然后点击 **检索**。也可以点击 **上传 PDF** 选择电脑中的论文。
2. 检索到多个结果时，选择正确的论文。论文载入后，左侧会显示原始 PDF；仅载入 PDF 不会自动开始翻译。
3. 点击右侧顶部的 **生成设置** 将其展开。程序会自动填写论文标题和输出文件名，通常保持 **自动选择** 即可。
4. 如有需要，可填写页码范围、调整并行数，或者在 **已下载论文** 中确认当前 PDF。首次使用建议保留默认值。
5. 点击 **生成译文**。生成过程会调用你在 `.env` 中配置的 DeepSeek API，并产生相应费用；关闭页面或中断后，再次生成会尽量从已完成的批次继续。
6. 生成完成后，中文译文会显示在右侧。公式使用 KaTeX 渲染，章节、图表和文献引用可点击跳转。
7. 页面左上方的 **已缓存论文** 下拉栏会列出本机已保存的论文。选择“已有译文”的论文会同时载入 PDF 与译文；“仅 PDF”的论文仍需按上述步骤生成译文。
8. 点击译文段落旁的 **提问**，或右下角的 **问答**，可以针对当前论文提问；回答会附带可跳转的证据段落。

论文、译文和问答记录保存在解压目录的 `data/` 中。再次启动同一目录时会继续使用这些缓存，不需要重复下载。
